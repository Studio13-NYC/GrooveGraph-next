...
2. Resolve connection mentally: same env as **`product/src/lib/server/config.ts`** / **`scripts/lib/typedb-env.mjs`** (`product/.env.local`); use **read** `oneShotQuery` or an explicit read transaction for inventory-style work.
